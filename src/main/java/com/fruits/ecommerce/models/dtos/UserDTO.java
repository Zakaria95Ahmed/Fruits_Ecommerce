...
public class UserDTO {
    ...
    @Size(min = 8, max = 100, message = "Password must be between 8 and 100 characters", groups = CreateUser.class)
    ...
    @JsonProperty(access = JsonProperty.Access.READ_ONLY)
    private boolean active; // Changed from isActive

    @JsonProperty(access = JsonProperty.Access.READ_ONLY)
    private boolean locked; // Changed from isNotLocked to represent the actual state
